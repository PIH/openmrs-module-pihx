...
public class DataChangeTest extends BaseModuleContextSensitiveTest {
	...
	public void shouldRecordPatientInserts() {
		assertThat(th.getTransactions().size(), is(0));
	    createTestPatient();
	...
	@Test
	@NotTransactional
	public void shouldNotRecordChangesIfErrorOccurs() {
		assertThat(th.getTransactions().size(), is(0));
		Patient p = createTestPatient();
		assertThat(th.getTransactions().size(), is(1));
		Exception identifierException = null;
		try {
			PatientIdentifier pi = new PatientIdentifier();
			pi.setIdentifier("ABC123");
			p.addIdentifier(pi);
			patientService.savePatient(p);
		}
		catch (Exception e) {
		...
		assertNotNull(identifierException);
		assertThat(th.getTransactions().size(), is(1));
	}
